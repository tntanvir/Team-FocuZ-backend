...
class AllUsersView(APIView):
    ...
    #     # Ensure only admin can change 'is_active'
    #     if 'is_active' in request.data and not request.user.is_staff:
    #         return Response({"error": "You do not have permission to change the user's status."}, status=status.HTTP_403_FORBIDDEN)

    #     # Serializer to update user information (role, name, phone, address)
    #     serializer = UserRoleUpdateSerializer(user, data=request.data, partial=True)
